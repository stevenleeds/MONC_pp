...
class opconfig():
    ...
    def update(self,cfgfile):
        ...
        self.dx=config.getfloat("domain","dx")
        self.dy=config.getfloat("domain","dy")
...
class sconfig():
    def __init__(self):
        pass
    def autoupdate(self,case,exper,filenumber,overwrite):
        if 'see' in hostname and 'leeds' in hostname: ## UNIVERSITY OF LEEDS
            self.update('see.syscfg',case,exper,filenumber,overwrite)
        elif 'arc2' in hostname and 'leeds' in hostname: ## UNIVERSITY OF LEEDS
            self.update('arc2.syscfg',case,exper,filenumber,overwrite)
        elif 'xcm' in hostname: ## Met Office monsoon
            self.update('xcm.syscfg',case,exper,filenumber,overwrite)
        elif 'nid' in hostname: ## Met Office monsoon
            self.update('xcm.syscfg',case,exper,filenumber,overwrite)
        else: ## e.g. laptop
            self.update('laptop.syscfg',case,exper,filenumber,overwrite)
    def do_replace(self,targ):        
        ...
        targ=targ.replace("$filenumber",str(self.filenumber))
        return targ
    def update(self,cfgfile,case,exper,filenumber,overwrite):
        config = ConfigParser.RawConfigParser()
        config.read(cfgfile)
        self.case=case
        self.exper=exper
        self.filenumber=int(filenumber)
